...
4. **Start the MCP server** (in separate terminal):
   ```bash
   python -m fastmcp run agentic/server/tools/
   ```
...
- **Cell 1-5**: Setup and orchestrator initialization
- **Cell 6**: Main orchestrator execution (`await chat_interface(orchestrator, "1")`)
- **Cell 7-8**: View conversation history and state traces
- **Cell 9**: Routing tests with sample tickets (KB hit/miss, escalation scenarios)
- **Cell 10**: Direct tool usage tests (lookup_customer, lookup_reservation, issue_refund)
- **Cell 11-13**: Persistent memory & personalization demo (first visit → resolution → return visit)
...
│   ├── server/                     # FastMCP server (run separately)
│   │   └── tools/
│   │       ├── kb_tools.py         # KB search server
│   │       ├── crm_tools.py        # CultPass DB queries
│   │       └── ticket_tools.py     # Ticket DB + memory operations
│   └── design/
│       └── agent_responsibilities.md  # Architecture documentation
└── README.md                       # This file
```
