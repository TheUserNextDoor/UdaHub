...
class CustomerMemory(Base):
    ...
    # JSON-encoded list of past resolutions
    past_resolutions = Column(Text, nullable=False, default="[]")
    
    # JSON-encoded list of past issue types
    past_issue_types = Column(Text, nullable=False, default="[]")
    ...
    __table_args__ = (
        UniqueConstraint('account_id', 'external_user_id', name='uq_customer_memory_per_account'),
    )
    
    def __repr__(self):
        return f"<CustomerMemory(external_user_id='{self.external_user_id}', past_issues={self.past_issue_types}')>"
